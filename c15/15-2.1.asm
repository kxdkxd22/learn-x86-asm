...
load_relocate_program:                      ;加载并重定位用户程序
  ...
  .b4:
         ...
         mov [es:esi+0x10],cx               ;登记LDT选择子到TCB中RPL=0
         ...
         mov dx,[es:edi+0x14]               ;登记程序代码段选择子cs到TS
         mov [es:ecx+76],dx

         mov dx,[es:edi+0x08]          ;登记用户程序堆栈段选择子到TSS中
         mov [es:ecx+80],dx

         mov dx,[es:edi+0x04]     ;登记用户程序数据段（DS）选择子到TSS中
         mov word [es:ecx+84],dx     ;指向用户程序头部段
...
start:
  ...
  .b3:
         ...
         ;为内核中的程序管理器的TSS分配内存空间
         mov ecx,104                 ;为该任务TSS分配内存
         call sys_routine_seg_sel:allocate_memory
         mov [progman_tss+0x00],ecx  ;保存程序管理器的TSS基地址


         ;在程序管理器的TSS中设置必要的项目
         mov word [es:ecx+96],0      ;没有LDT，处理器允许没有LDT的任务
         mov word [es:ecx+102],103   ;没有I/O位图，0特权级上不需要
         mov word [es:ecx+0],0       ;反向链=0
         mov dword [es:ecx+28],0     ;登记CR3（PDBR）
         mov word [es:ecx+100],0     ;不需要0、1、2特权级的堆栈，0特权级不会向低特权级转移控制


         ;创建TSS描述符，并安装到GDT中
         mov eax,ecx                 ;TSS的起始线性地址
         mov ebx,103                 ;段长度（界限）
         mov ecx,0x00408900          ;TSS描述符特权级 DPL=0
         call sys_routine_seg_sel:make_seg_descriptor
         call sys_routine_seg_sel:set_up_gdt_descriptor
         mov [progman_tss+0x04],cx   ;保存程序管理器的TSS描述符选择子

         ;任务寄存器TR中的内容是任务存在的标志，该内容也决定了当前任务是谁
         ;下面的指令为当前正在执行的0特权级任务“程序管理器”后补手续
         ltr cx
         ...
         jmp far [es:ecx+0x14]         ;执行任务切换任务A

         ;创建用户程序B的任务门描述符
         ;mov bx,[es:ecx+0x18]
         ;mov cx,1_11_00101_00000000B
         ;call sys_routine_seg_sel:make_gate_descriptor
         ;call sys_routine_seg_sel:set_up_gdt_descriptor
         ;mov [salt_5+260],cx           ;将任务门选择子装载到C-SALT表中
