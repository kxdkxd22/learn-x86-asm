...
core_base_address equ 0x00040000  ;内核加载的起始物理地址
...
SECTION mbr vstart=0x00007c00
        ...
        mov dword [ebx+0x08],0x0000ffff    ;基地址为0，界限为0xFFFFF，DPL=0
        mov dword [ebx+0x0c],0x00cf9800    ;4KB粒度，代码段描述符，向上扩展

        ;创建2#描述符，保护模式下的数据段和堆栈段描述符
        mov dword [ebx+0x10],0x0000ffff    ;基地址为0，界限为0xFFFFF，DPL=0
        mov dword [ebx+0x14],0x00cf9200    ;4KB粒度，数据段描述符，线上扩展
        ...
        ;以下进入保护模式......
        jmp dword 0x0008:flush   ;16位GDT段描述符选择子：32位偏移地址
                                 ;请流水线并串行化处理器

        [bits 32]

flush:
        mov eax,0x00010          ;GDT中数据段选择子
        mov ds,eax
        mov es,eax
        mov fs,eax
        mov gs,eax
